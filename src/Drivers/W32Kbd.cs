...
namespace TurboVision
{
    ...
    public class W32Kbd
    {
        ...
        {
            byte[] CtrlTable = new byte[13] { 119, 141, 132, 142, 115, 143, 116, 144, 117, 145, 118, 146, 147 };
            byte[] AltTable = new byte[13] { 151, 152, 153, 74, 155, 76, 157, 78, 159, 160, 161, 162, 163 };
        ...
        public static void KbdUpdateEventQueues()
        {
            ...
            INPUT_RECORD InRec = new INPUT_RECORD();
            bool FoundAlt;
            byte AltNumeric = 0;
            ...
            do
            {
                ...
                Windows.ReadConsoleInput(Windows.GetStdHandle((int)StdHandles.STD_INPUT_HANDLE),
                    ref InRec, out EventCount);
                switch ((EventTypes)InRec.EventType)
                {
                    case EventTypes.KEY_EVENT:
                        if (SysKeyCount <= SysKeyQue.GetUpperBound(0))
                            if (InRec.KeyEvent.bKeyDown)
                            {
                                SysKeyQue[SysKeyCount].wKeyEvent = InRec.KeyEvent;
                                SysShiftState = SysKeyQue[SysKeyCount].skeShiftState;
                                switch ((int)InRec.KeyEvent.wVirtualKeyCode)
                                {
                                    case VK_SHIFT:
                                    case VK_CONTROL:
                                    case VK_MENU:
                                    ...
                                    default:
                                        ...
                                        byte[] bb = enc.GetBytes(new char[] { InRec.KeyEvent.UnicodeChar });
                                        SysKeyQue[SysKeyCount].skeKeyCode = TranslateKeyCode((byte)InRec.KeyEvent.wVirtualKeyCode, (byte)InRec.KeyEvent.wVirtualScanCode, /*InRec.KeyEvent.AsciiChar*/ bb[0], InRec.KeyEvent.dwControlKeyState);
                                        SysKeyQue[SysKeyCount].wKeyEvent = InRec.KeyEvent;
                                        if (SysKeyQue[SysKeyCount].skeKeyCode == 0)
                                            return;
                                        FoundAlt = false;
                                        if (((SysKeyQue[SysKeyCount].skeShiftState & 0x08) == 0x08) &&
                                            ((InRec.KeyEvent.dwControlKeyState & 0x100) == 0))
                                            if (SysPlatform == 1)
                                            {
                                                for (int i = 0; i < 10; i++)
                                                ...
                                                if (((int)InRec.KeyEvent.wVirtualKeyCode >= VK_NUMPAD0) && ((int)InRec.KeyEvent.wVirtualKeyCode <= VK_NUMPAD9))
                                                {
                                                    AltNumeric = (byte)((AltNumeric * 10) + InRec.KeyEvent.wVirtualKeyCode - VK_NUMPAD0);
                                                    FoundAlt = true;
                                                }
                                        if (!FoundAlt)
                                        {
                            ...
                            {
                                if ((int)InRec.KeyEvent.wVirtualKeyCode == VK_MENU)
                                    if (AltNumeric != 0)
                                    {
                    ...
                    case EventTypes.MOUSE_EVENT:
                        ...
                        {
                            SysMouQue[SysMouCount].smePos.X = InRec.MouseEvent.dwMousePosition.X;
                            SysMouQue[SysMouCount].smePos.Y = InRec.MouseEvent.dwMousePosition.Y;
                            SysMouQue[SysMouCount].smeButtons = (byte)InRec.MouseEvent.dwButtonState;

...
namespace TurboVision.Menus
{
	public class MenuBox : MenuView
	{
		...
		internal void FrameLine(int N, DrawBuffer B, byte CNormal, byte Color)
		{
			B.FillBuf( ldMenuFrameChars.Substring( N, 2), (byte)CNormal, 2);
			B.FillChar( (char)ldMenuFrameChars[N + 2], (char)Color, (int)(Size.X - 4), 2);
			B.FillBuf( ldMenuFrameChars.Substring( N + 3, 2), (byte)CNormal, 2, (int)(Size.X - 2));
		}
		...
		public override void Draw()
		{
			...
			Y = 0;
			Color = CNormal;
			FrameLine(0, B, (byte)CNormal, (byte)Color);
			DrawLine( ref Y, B);
			if( Menu != null)
			{
				P = Menu.Items;
				while( P != null)
				{
					...
					else
					{
						if( P.Disabled)
							...
							if( P == Current)
							Color = CSelect;
						FrameLine(10, B, (byte)CNormal, (byte)Color);
						B.FillCStr( P.Name, Color, 3);
						if( P.Command == 0)
							B.FillChar( (char)ldSubMenuArrow, (char)Color, 1, (int)Size.X - 4);
						else
							if( P.Param != "")
							B.FillStr( P.Param, (char)Color, (int)Size.X - 3 - P.Param.Length);
					}
					DrawLine( ref Y, B);
					P = P.Next;
				}
			...
			Color = CNormal;
			FrameLine(5, B, (byte)CNormal, (byte)Color);
			DrawLine( ref Y, B);
		}
	}
}

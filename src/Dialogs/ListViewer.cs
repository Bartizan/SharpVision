...
namespace TurboVision.Dialogs
{
	...
	public abstract class ListViewer : View
	{
		...
		public static char[] CListViewer = ("\x1a\x1a\x1b\x1c\x1d").ToCharArray();
		...
		{
			int Item;
			uint NormalColor, SelectedColor, FocusedColor, Color;
			...
			for( int i = 0; i < Size.Y; i++)
			{
				for( int j = 0; j < NumCols; j++)
				{
					...
					B.FillChar( (char)' ', (char)Color, ColWidth, CurCol);
					if( Item < Range)
					{
						Text = GetText( Item, ColWidth + Indent);
						Text = (Text + new string(' ', ColWidth)).Substring( Indent, ColWidth);
						B.FillStr( Text, (char)Color, (int)CurCol + 1);
						if( ShowMarkers)
						{
							B.drawBuffer[CurCol].AsciiChar = (char)( SpecialChars[SCOff]);
							B.drawBuffer[CurCol + ColWidth - 2].AsciiChar = (char)( SpecialChars[SCOff + 1]);
						}
					}
					B.FillChar( (char)ldVerticalBar, (char)GetColor(5), 1, (int)CurCol + ColWidth - 1);
				}

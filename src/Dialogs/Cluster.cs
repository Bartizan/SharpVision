...
namespace TurboVision.Dialogs
{
	...
	public class Cluster : View
	{
		...
		private static char[] CCluster = ("\x10\x11\x12\x12\x1F").ToCharArray();
		...
		public void DrawMultiBox( string Icon, string Marker)
		{
			...
			{
				B.FillChar( (char)' ', (char)CNorm, (int)Size.X);
					...
					if( Cur < Strings.Count)
					{
							...
							( B.drawBuffer.Length)) && ( Col < Size.X))
						{
							if( ! ButtonState(Cur))
								...
								if( (Cur == Sel) && ( (State & StateFlags.sfFocused) != 0))
								Color = CSel;
							else
								Color = CNorm;
							B.FillChar( (char)' ', (char)Color, (int)Size.X - Col, Col);
							B.FillStr( Icon, (char)Color, Col);
							B.drawBuffer[Col + 2].AsciiChar = (char)Marker[MultiMark(Cur)];
							B.FillCStr( Strings[Cur], (char)Color, Col + 5);
							if( ShowMarkers && ( ( State & StateFlags.sfFocused) != 0) && ( Cur == Sel))
							{
                    ...
                    default:
                        {
                            for (int I = 0; I < Strings.Count; I++)
                            {
                                ...
                                (((Owner.Phase == Phases.phPostProcess) || ((State & StateFlags.sfFocused) != 0))
                            ...
                            if ((Event.CharCode == ' ') && ((State & StateFlags.sfFocused) != 0) && ButtonState(Sel))
                            {
                                Press(Sel);
                                DrawView();
                                ClearEvent(ref Event);
        ...
        public override void SetState(View.StateFlags AState, bool Enable)
        {
            ...
            if (AState == StateFlags.sfFocused)
                DrawView();
        }

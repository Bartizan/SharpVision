...
namespace TurboVision.Dialogs
{
	...
	public class InputLine : View
	{
		...
		private static char[] CInputLine = ("\x13\x13\x14\x15").ToCharArray();
        ...
        public InputLine():base( new Rect( 0, 0, 30, 1))
        {
        }

        public InputLine( Rect Bounds, int AMaxLen):base( Bounds)
		{
		...
		public override void Draw()
		{
			...
			B.FillStr( (Data + new string(' ', (int)Size.X)).Substring( FirstPos, (int)(Size.X - 2)), (char)Color, 1);
			if( CanScroll(1))
					...
					case Event.KeyDown :
                        ...
                        switch (Event.KeyCode)
                        {
                            ...
                            case KeyboardKeys.Ins:
                                SetState(StateFlags.sfCursorIns, (State & StateFlags.sfCursorIns) == 0);
                                break;
		...
		public override void SetState(StateFlags AState, bool Enable)
		{
			...
			if( (AState == StateFlags.sfSelected) ||
				( ( AState == StateFlags.sfActive) && ( (State & StateFlags.sfSelected) != 0)))
				SelectAll( Enable);
			else

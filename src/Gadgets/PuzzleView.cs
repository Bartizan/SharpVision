...
namespace TurboVision.Gadgets
{
	public class PuzzleView : View
	{
		...
		private static char[] CPuzzleView = ("\x06\x07").ToCharArray();
...
//			for ( int i = 0; i < Board.GetUpperBound(0); i++)
		...
		public override void Draw()
		{
			...
			for( int i = 0; i < 4; i++)
			{
				B.FillChar( (char)' ', (char)ColorBack, 18);
				if( i == 1)
					B.FillStr( "Move", (char)ColorBack, 13);
				if( i == 2)
				{
					S1 = string.Format("{0:G3}", Moves);
					B.FillStr( S1, (char)ColorBack, 14);
				}
				for( int j = 0; j < 4; j++)
				{
					S1 = " " + (char)Board[ i, j] + " ";
					if( Board[ i, j] == ' ')
						B.FillStr( S1, (char)Color[0], j * 3);
					else
						B.FillStr( S1, (char)Color[Map[Board[i, j]-65]], j * 3);
				}

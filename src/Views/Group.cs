...
namespace TurboVision.Views
{
	...
	public class Group : View
	{
		...
		public Group( Rect Bounds):base( Bounds)
		{
			...
			if( Current != null)
			{
				...
				do
				{
					if( Forwards)
						...
						P = P.Prev;
				}while( !((((P.State & (StateFlags.sfVisible | StateFlags.sfDisabled)) == StateFlags.sfVisible) &&
		...
		{
			ForEach( new ForEachProc( DoAwaken));
		}
		...
		public bool ContainsMouse( View P, Point Where)
		{
			return (( (P.State & StateFlags.sfVisible) != 0) &&
				P.MouseInView( Where));
		}
		...
		{
			if( ((State & StateFlags.sfFocused) != 0) && ( P != null))
		...
		{
			if( (P == null) || ( ((P.State & StateFlags.sfDisabled) != 0) && ( ( E.What & (PositionalEvents | FocusedEvents))) != 0))
				return;
			switch( Phase)
			{
		...
		public override void SetData(params object[] Rec)
		{
			uint I = 0;
			View V;
			if( Last != null)
		...
		public void DoExpose( View P, params object[] o)
		{
			if( (State & StateFlags.sfVisible) != 0)
				P.SetState( StateFlags.sfExposed, (bool)o[0]);
		}

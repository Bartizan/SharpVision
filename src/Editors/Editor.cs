...
namespace TurboVision.Editors
{
    public class Editor : View
    {
        ...
        public static char[] CEditor = ("\x06\x07").ToCharArray();
        ...
        public void SetCmdState(int Command, bool Enable)
        {
            ...
            if (Enable && ((State & StateFlags.sfActive) != 0))
                EnableCommands(S);
            else
                DisableCommands(S);
        }
            ...
            while (EndPos > idxPos)
            {
                ...
                switch (buffer[P])
                {
                    ...
                    default:
                        ...
                        DrawBuf.drawBuffer[OutPtr].Attribute = (char)attr;
                        OutPtr++;
                        break;
                }
                P++;
        ...
        private void FillSpace(int i, ref int OutCnt, ref int OutPtr, ushort attr, DrawBuffer DrawBuf)
        {
            ...
            while (i > 0)
            {
                DrawBuf.drawBuffer[OutPtr].AsciiChar = (char)' ';
                DrawBuf.drawBuffer[OutPtr].Attribute = (char)attr;
                OutPtr++;
                i--;
            }
        ...
        public override void SetState(View.StateFlags AState, bool Enable)
        {
            ...
            switch (AState)
            {
                ...
                case StateFlags.sfFocused :
                    if ((State & StateFlags.sfSelected) == StateFlags.sfSelected)
                    {
                        if (HScrollBar != null)
